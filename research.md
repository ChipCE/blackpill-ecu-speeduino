...
- `pref/datasheet/SP721.pdf`, `pref/datasheet/sn74ahct541.pdf` — could not text-extract (missing `poppler-utils`); details below on these two parts are from general datasheet knowledge, not verified against the local PDFs
...
- Either add a resistor divider + buffer per channel (as pazi88 did) and keep sensor excitation at 5V, or re-reference sensor dividers to 3.3V from the start (simpler, but changes calibration math for ratiometric sensors — factor into firmware).
...
- This divider only sets normal-range scaling — it provides no transient protection. The TVS clamp on the raw 12V-SW sense line (upstream of the divider, per the dual-layer protection plan above) is still required for load-dump survival.
...
### Bluetooth (HC-05) pin assignment
- **`PA9` → HC-05 RXD, `PA10` ← HC-05 TXD** (USART1). Free of all the allocations above, and FT-rated (5V-tolerant) for headroom against a 5V-powered HC-05 module's TX level.
- **Confirmed against the actual firmware**, not just picked freely: `speeduino/speeduino/board_stm32_official.h:126-127` already hardcodes `HardwareSerial Serial1(PA10, PA9);` for Blackpill-class targets (`SERIAL_UART_INSTANCE==2`) — this is Speeduino's dedicated wireless/Bluetooth serial port. Wiring the HC-05 to `PA9`/`PA10` matches the stock firmware exactly, no source changes needed.
- The same file's `pinIsReserved()` (line 134-140, for `ARDUINO_BLACKPILL_F401CC`/`F411CE`) independently confirms `PA11`, `PA12`, `PC14`, `PC15` as reserved — matches the schematic findings above.
- General takeaway: STM32 pin-to-peripheral assignment is a **compile-time** decision (which physical pins back a given `Serial`/`SPI`/etc. object), not a runtime or TunerStudio-configurable setting — changing it later means editing the board file and recompiling, not just rewiring.
...
- Bluetooth/serial header (`PA9`/`PA10`), SD/SPI-flash logging, RTC crystal + backup battery
...
- Confirm exact SP721 and SN74AHCT541 electrical specs against their datasheets directly (text extraction failed locally; `poppler-utils` not installed for `pdftoppm`, though a rendering workaround via `pymupdf` worked for schematics). ~~TC4423A/TC4424A and ULN2003A~~ — resolved, both datasheets added and verified.
